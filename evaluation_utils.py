...
def shuffle_all(predictions, plaintexts, keys):
    """Shuffle predictions, plaintexts, and keys together."""
...
def get_log_prob(predictions, plaintext):
    """Compute log probability for key guessing."""
...
def compute_key_rank(predictions, plaintexts, keys):
    """Compute key rank (guessing entropy) for ASCAD datasets.

    Args:
        predictions: Model predictions [N, 256]
        plaintexts: Plaintext bytes [N]
        keys: True key bytes [N]
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("Usage: python evaluation_utils.py <data_path>")
        sys.exit(1)
    ...
    for i in range(10):
        ...
        print(f"{label}/{sbox[np.bitwise_xor(ptest, key)]}/{key}")
